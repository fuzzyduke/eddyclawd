...
def master_deploy_v2():
    ...
    try:
        ...
        print("--- FORCING GIT SYNC TO 2d144ac ---")
        ssh.exec_command("cd /srv && git fetch origin master && git reset --hard origin/master && git clean -ffdx")
        time.sleep(3)
        
        # 2. Re-trigger Infra (Now using standard docker-compose.yml)
        print("--- RESTARTING TRAEFIK WITH RECTIFIED FILENAME & API VERSION ---")
        ssh.exec_command("cd /srv/infra/traefik && docker compose down && docker compose up -d")
        time.sleep(10) # Give Traefik time to start and parse Docker
